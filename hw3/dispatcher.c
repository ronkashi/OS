...
void my_signal_handler( int signum, siginfo_t* info,void* ptr)
{
    ...
    close(fdPipe);
    if(0 > unlink(pipe_name)){
    	printf("Error in unlink pipe file: %s\n", strerror(errno));
    	flagSigHandler = RV_ERROR_I_O_FILES;
    }
}
///////////////////////////////////////////////////////////
int main(int argc, char *argv[])
{
	...
	char time_to_sleep[FileNameLength];
	...
	for (i = 0; (i < forks_num) && (RV_SUCCESS == flagSigHandler); ++i)
	{
		...
		if (pid_children[i] == 0){
			...
			sprintf(time_to_sleep,"%d",i);
			char *arg_for_execv[] = {"counter", argv[1], argv[2], offset_str, len_of_chunk_str,time_to_sleep, NULL};
